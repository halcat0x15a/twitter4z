...
class TimelinesSpec extends Specification { def is =
  ...
  "statuses/retweeted_to_user should"                                             ^
    "contain 1 status when set 1 for count"                                       ! RetweetedToUser.e1^
  ...
  "statuses/retweeted_by_user should"                                             ^
    "contain 1 status when set 1 for count"                                       ! RetweetedByUser.e1^
  ...
  case object UserTimeline {
    ...
    lazy val e1 = statuses1 must have size 1
    lazy val id = statuses1.last.id
    lazy val statuses2 = userTimeline(ID("halcat0x15a"), paging=Paging(sinceId=id, maxId=id)).unsafe
    lazy val e2 = statuses2 must be empty
  ...
  case object RetweetedToUser {
    lazy val statuses1 = retweetedToUser(ID("halcat0x15a"), Paging(count=1)).unsafe
    lazy val e1 = statuses1 must have size 1
    lazy val id = statuses1.last.id
    lazy val statuses2 = retweetedToUser(ID("halcat0x15a"), paging=Paging(sinceId=id, maxId=id)).unsafe
    lazy val e2 = statuses2 must be empty
    lazy val e3 = forall (statuses1) { status =>
  ...
  case object RetweetedByUser {
    lazy val statuses1 = retweetedByUser(ID("halcat0x15a"), Paging(count=1)).unsafe
    lazy val e1 = statuses1 must have size 1
    lazy val id = statuses1.last.id
    lazy val statuses2 = retweetedByUser(ID("halcat0x15a"), paging=Paging(sinceId=id, maxId=id)).unsafe
    lazy val e2 = statuses2 must be empty
    lazy val e3 = forall (statuses1) { status =>
      status.user.screenName must be equalTo "halcat0x15a"
    }
